<template>
  <teleport to="body">
    <div
      ...
      @click="handleModalToglle({ status: false })"
    >
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      modal.listen(handleModalToglle);
    });

    onBeforeMount(() => {
      modal.off(handleModalToglle);
    });

    function handleModalToglle(payload) {
      if (payload.status) {
    ...
    return { state, handleModalToglle };
  },
};
</script>
